...
int main() {
  ...
  auto f = [](int n, int d) { return choose(n - 2 + d, d); };

  // O(m^2) solution
  // mint ans = 0;
  // rep (d_odd, m + 1) {
  //   rep (d_even, m + 1) {
  //     ans += (2 * m + 1 - d_odd - d_even) * f((n + 2) / 2, d_odd) * f((n + 1) / 2, d_even);
  //   }
  // }
  // cout << ans << endl;

  // O(m) solution
  mint fd_odd = 0, fd_even = 0, dfd_odd = 0, dfd_even = 0;
  rrep (d, m + 1) {
    fd_odd += f((n + 2) / 2, d);
    fd_even += f((n + 1) / 2, d);
    dfd_odd += d * f((n + 2) / 2, d);
    dfd_even += d * f((n + 1) / 2, d);
  }
  mint ans = (2 * m + 1) * fd_odd * fd_even - dfd_odd * fd_even - fd_odd * dfd_even;
  cout << ans << endl;
