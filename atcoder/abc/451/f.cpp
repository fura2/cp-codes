...
int main() {
  ...
  rep (_, q) {
    ...
    if (U.is_same(2 * u, 2 * v + 1)) {
      cout << ans << "\n";
      continue;
    }

    int ku = min(U.value(2 * u), U.value(2 * u + 1));
    int kv = min(U.value(2 * v), U.value(2 * v + 1));
    U.unite(2 * u, 2 * v + 1);
    U.unite(2 * v, 2 * u + 1);

    if (U.is_same(2 * u, 2 * u + 1)) {
      cout << -1 << "\n";
      ng = true;
    }
    else {
      ans = ans - ku - kv + min(U.value(2 * u), U.value(2 * u + 1));
      cout << ans << "\n";
    }
  }
