...
int main() {
    ...
    // dp[i][j] = (a_0, ..., a_{i-1}) の (同じ値も区別した) 並べ替えであって、次の条件を満たすものの個数
    //            条件: b = (0, a_0, a_1, ..., a_{i-1}, 0) に対して
    //                  b_k < b_{k+1} なる k はちょうど j 個ある
    vector dp(n + 1, vector(n + 1, mint(0)));
    dp[0][0] = 1;
    rep(i, n) {
        int same = 0;
        rep(j, i) if (a[j] == a[i]) same++;

        rep(j, i + 1) {
            // x > b_k >= b_{k+1} なる b_k と b_{k+1} の間に x (= a[i]) を挿入
            if (j < n) {
                dp[i + 1][j + 1] += max((i + 1) - j - same, 0) * dp[i][j];
            }
            // それ以外の場所に x を挿入
            dp[i + 1][j] += (j + same) * dp[i][j];
        }
    }
    cout << dp[n][K + 1] << endl;
