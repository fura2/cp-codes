...
int main(){
	...
	rep(i,n) scanf("%d%d%d%d",&a[i],&b[i],&c[i],&d[i]), a[i]--, b[i]--;

	int N=h+2*n+w+2;
	int s=N-2,t=N-1;
	mf_graph<int> G(N);
	rep(i,h) G.add_directed_edge(s,i,1);
	rep(i,n){
		for(int j=a[i];j<c[i];j++){
			G.add_directed_edge(j,h+i,1);
		}
		G.add_directed_edge(h+i,h+n+i,1);
		for(int j=b[i];j<d[i];j++){
			G.add_directed_edge(h+n+i,h+2*n+j,1);
		}
	}
	rep(i,w) G.add_directed_edge(h+2*n+i,t,1);
