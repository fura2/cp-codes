...
int main(){
	...
	mint ans=0;
	int small=0;
	for(auto p:run_length_encoding(a)){
		int large=n-p.second-small;
		ans+=(choose(p.second+small,k)-choose(small,k))*p.first;
		ans-=(choose(p.second+large,k)-choose(large,k))*p.first;
		small+=p.second;
	}
	printf("%d\n",ans.to_int());
