...
lint f(const string& s, char a, char b) {
  lint res = 0;
  lint d = 0;
  map<lint, lint> h;
  h[0] = 1;
...
lint g(const string& s) {
  lint res = 0;
  lint cnt[128] = {};
  map<tuple<lint, lint, lint>, lint> h;
  h[{0, 0, 0}] = 1;
  for (char c: s) {
    cnt[c]++;
    lint mn = min({cnt['A'], cnt['B'], cnt['C']});
    tuple<lint, lint, lint> t{cnt['A'] - mn, cnt['B'] - mn, cnt['C'] - mn};
    res += h[t]++;
  }
  return res;
}
...
int main() {
  ...
  lint bc = f(s, 'B', 'C');
  lint abc = g(s);
  cout << n * (n + 1) / 2 - ab - ac - bc + 2 * abc << endl;
