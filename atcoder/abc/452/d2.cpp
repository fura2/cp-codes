// solution based on https://atcoder.jp/contests/abc452/editorial/18420
...
int main() {
  ...
  vector pos(128, vector<int>());
  rep (i, n) pos[s[i]].emplace_back(i);
  rep (c, 'a', 'z' + 1) pos[c].emplace_back(n);

  lint res = 0;
  rep (i, n) {
    int k = i - 1;
    for (char c: t) {
      k = *upper_bound(all(pos[c]), k);
      if (k == n) break;
    }
    res += n - k;
  }
  cout << lint(n) * (n + 1) / 2 - res << endl;
