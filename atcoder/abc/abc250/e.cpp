...
int main() {
  ...
  vector<int> p(q);
  ranges::iota(p, 0);
  ranges::sort(p, [&](int i, int j) { return x[i] < x[j]; });

  vector<bool> ans(q);
  int idx = 0, l = 0, r = 0;
  unordered_set<int> A, Bl, Br;
  rep (i, n) {
    A.emplace(a[i]);
    while (r < n && A.contains(b[r])) {
      Br.emplace(b[r]);
      ++r;
    }
    while (l < n && A.contains(b[l]) && Bl.size() < A.size()) {
      Bl.emplace(b[l]);
      ++l;
    }

    while (idx < q && x[p[idx]] == i) {
      ans[p[idx]]
          = Br.size() == A.size() && l - 1 <= y[p[idx]] && y[p[idx]] < r;
      idx++;
    }
  }
